...
function reportGenerator() {
  ...
  loadResource('https://cdn.jsdelivr.net/npm/vue@3/dist/vue.global.prod.js', () => {
    ...
    const columns = [
      ...
      {name: 'Repos.#ID', sort: {compare: (a, b) => a.props.content.replace(/<[^>]*>?/gm, '').localeCompare(b.props.content.replace(/<[^>]*>?/gm, ''))}, attributes: {style: 'width:150px', class: 'gridjs-td ellipsis-start'}},
      {name: 'Title', sort: true, attributes: {style: 'gridjs-td ellipsis'}},
      {name: 'Labels', sort: true, hidden: true, attributes: {style: 'width:150px', class: 'gridjs-td ellipsis'}},
    ...
    function addExportButton(endDate) {
      ...
      const observer = new MutationObserver((mutations) => {
        if (mutations.some(mut => mut.target === document.querySelector('.gridjs-tbody'))) {
          observer.disconnect();
          const gridjsTh = document.querySelectorAll('.gridjs-thead th');
          for (const th of gridjsTh) {
            th.className = 'gridjs-th gridjs-th-sort';
          }
        }
      });
      observer.observe(document.querySelector('.gridjs-table'), {childList: true, subtree: true});
    }
    ...
    window.app = createApp({
      ...
      methods: {
        ...
        filterRepos(repo) {
          const newData = window.grid.config.data.filter(row => row[3].props.content.replace('/<[^>]+>|#/g', '').includes(repo));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
        ...
        filterLabel(label) {
          const newData = window.grid.config.data.filter(row => row[5].props.content.replace('/<[^>]+>/g', '').includes(label));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
          addExportButton(this.endDate);
        },
        filterUser(createdUser, assignedUser) {
          const newData = window.grid.config.data.filter(row => createdUser && row[1].includes(createdUser) || assignedUser && row[2]?.includes(assignedUser));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
          addExportButton(this.endDate);
        ...
        async queryReport(e) {
          ...
          if (this.by === 'myself') {
            ...
            if (!commentsColumn) {
              columns.push({
                ...
                attributes: {style: 'white-space:nowrap;overflow:hidden;text-overflow:ellipsis'},
              });
            }
          ...
          for (const issue of resData || []) {
            ...
            const repoName = issue.repository.full_name;
            reposSummaryData[repoName] = reposSummaryData[repoName] || {count: 0, open: 0, closed: 0};
            reposSummaryData[repoName].count++;
            this.reposSummary = reposSummaryData;
            ...
            for (let j = 0; j < labels.length; j++) {
              ...
              label += `<span style="background-color:#${labels[j].color}" class="ui label" data-tooltip-content="${labels[j].name}">${labels[j].name}</span> `;
            ...
            usersSummaryData[issue.user.full_name] = usersSummaryData[issue.user.full_name] || {created: 0, assigned: 0};
            const assigneeNames = issue.assignees?.map((a) => a.full_name) || [];
            for (const assignee of assigneeNames) {
              usersSummaryData[assignee] = usersSummaryData[assignee] || {created: 0, assigned: 0};
              usersSummaryData[assignee].assigned++;
            }
            usersSummaryData[issue.user.full_name].created++;
            ...
            // eslint-disable-next-line no-undef
            data.push([issue.pull_request ? 'PR' : 'Issues', issue.user.full_name, issue.assignees?.map((a) => a.full_name).join(', '), gridjs.html(issueID), issue.title, gridjs.html(label), issue.state, issue.created_at.slice(0, 19).replace('T', ' '), issue.updated_at.slice(0, 19).replace('T', ' '), `${pct}%`, issue.due_date ? issue.due_date.slice(0, 10) : '']);
            if (commData.length) {
              const maxLen = 100;
              const comm = commData.filter((c) => c.issue_url === issue.html_url);
              if (comm.length) {
                const commLen = comm[comm.length - 1].body.length;
                const lastCommentBody = cleanCellText(comm[comm.length - 1].body.slice(0, maxLen));
                const commentSignatureIndex = lastCommentBody.lastIndexOf(userSignature);
                const commentText = commentSignatureIndex === -1 ? lastCommentBody.slice(0, maxLen) : lastCommentBody.slice(0, commentSignatureIndex < maxLen ? commentSignatureIndex : maxLen);
                data[data.length - 1].push(commentText.trim() + (commLen > maxLen ? '...' : ''));
              }
            }
          }
          ...
          this.usersSummary = Object.fromEntries(Object.entries(usersSummaryData).sort((a, b) => b[1].count - a[1].count));
