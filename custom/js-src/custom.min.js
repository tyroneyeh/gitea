...
async function autoRemoveFiles() {
  ...
  for (const el of userComments) {
    ...
    if (updateDataSet) {
      ...
      const attachmentsSizes = Array.from(commentElement.querySelectorAll('.dropzone-attachments div.tw-flex .tw-text-text-light')).map((el) => parseSizeToBytes(el.textContent));
      let html;
