...
function goTo(position, e) {
  ...
  position === 'bottom' && setTimeout(() => window.scrollTo({top: Math.max(document.documentElement.scrollHeight, document.body.scrollHeight)}), 100);
  e.currentTarget.classList.add('tw-hidden');
}

function controlButtonsVisibility() {
  document.querySelector('.gotocontrol.up').addEventListener('click', (e) => {
    goTo('top', e);
  });
  document.querySelector('.gotocontrol.down').addEventListener('click', (e) => {
    goTo('bottom', e);
  });
 ...
 */
function hiToAssignee(userNames, isAppend) {
  ...
  const availableUserElements = Array.from(window.assigneeElement.querySelectorAll('.dropdown a.item span.username-display'), (el) => el.closest('a.item'));
  ...
  for (const availableUserElement of availableUserElements) {
    const canChecked = userNames.includes(availableUserElement.querySelector('.username-display').title);
    ...
    const isNewAssignee = !window.assigneeIds.some((el) => el.id === assigneeData.id);
    if (isNewIssue && checked !== canChecked) {
      window.assigneeElement = window.assigneeElement.querySelector('.dropdown');
      availableUserElement.click();
    } else if (isNewAssignee && ((checked !== canChecked && !isAppend) || (!checked && canChecked && isAppend))) {
...
async function autoLabels(content) {
  ...
  const allLabelSpans = Array.from(window.labelElement.querySelectorAll('.menu a.item span.gt-ellipsis, .menu a.item span.scope-parent'));
  const titleInputElement = document.querySelector('#issue_title');

  window.labelIds = window.labelIds || [];

  const getLabelText = (span) => {
    if (span.classList.contains('scope-parent')) {
      return Array.from(span.querySelectorAll('.scope-left, .scope-middle'))
  ...
  let detachSet = new Set();

  const hasSolved = attachSet.has('Solved');
  const hasTestOK = attachSet.has('Test/OK') || attachSet.has('TestOK');
  const hasTestNG = attachSet.has('Test/NG') || attachSet.has('TestNG');
  const hasNotTest = attachSet.has('NotTest');

  if (hasSolved) {
    ['Test/NG', 'TestNG', 'NotTest'].forEach(l => {
      detachSet.add(l);
      attachSet.delete(l);
    });
  } else if (hasTestNG || hasNotTest) {
    detachSet.add('Solved');
    attachSet.delete('Solved');
  }

  if (hasTestNG || hasNotTest) {
    ['Test/OK', 'TestOK'].forEach(l => {
      detachSet.add(l);
      attachSet.delete(l);
    });
  } else if (hasTestOK) {
  ...
  const processAction = (labelName, action) => {
    const spanEl = allLabelSpans.find((span) => getLabelText(span).toLowerCase() === labelName.toLowerCase());
    if (!spanEl) return;

    const aEl = spanEl.closest('a');
    const isChecked = aEl.classList.contains('checked');

    if ((action === 'attach' && isChecked) || (action === 'detach' && !isChecked)) return;

    const id = aEl.dataset.value;
    if (!window.labelIds.some(item => item.action === action && item.id === id)) {
      if (titleInputElement && action === 'attach') {
         window.labelElement = window.labelElement.querySelector('.dropdown') || window.labelElement;
      }
      window.labelIds.push({ action, id });
    }
    aEl.click();
  };

  attachSet.forEach(label => processAction(label, 'attach'));
  detachSet.forEach(label => processAction(label, 'detach'));
  ...
  if (titleInputElement) {
    ...
    if (titleLabelMatch) {
      const titleSpan = allLabelSpans.find((span) => getLabelText(span).toLowerCase() === titleLabelMatch.toLowerCase() && !span.closest('a').classList.contains('checked'));
      if (titleSpan) {
        window.labelElement = window.labelElement.querySelector('.dropdown') || window.labelElement;
        window.childLabelElements.push(titleSpan);
      }
    }

    const deptSpan = allLabelSpans.find((span) => getLabelText(span) === window.dept && !span.closest('a').classList.contains('checked'));
    if (deptSpan) {
      window.labelElement = window.labelElement.querySelector('.dropdown') || window.labelElement;
      window.childLabelElements.push(deptSpan);
    }
  ...
  for (const textarea of document.querySelectorAll('textarea')) {
    ...
    const doSave = (eventType) => {
      ...
      if (content.length) {
        ...
        const isAppend = firstLine.endsWith(':');
        if (firstLine.startsWith('Hi @') && (!firstLine.endsWith(',') || isAppend)) {
          hiToAssignee(firstLine.replace(/:$/, '').slice(4).toLowerCase().split(/[\s@]+/).filter(Boolean), isAppend);
...
async function autoSubscribe() {
  ...
  const form = isMuted?.closest('button') || muteButton?.closest('button');
  if (muteButton && form) {
    const assigneeHasSelf = document.querySelector('.relaxed').querySelector(`span[title="${window.loginedUser}"]`);
    if (assigneeHasSelf) {
      const res = await post(form.dataset.fetchUrl, {watch: 1});
  ...
  if (isMuted || muteButton) {
    ...
    if (watchParam) {
      post(form.dataset.fetchUrl, {watch: watchParam})
...
function markdownColorPicker() {
  document.addEventListener('change', function(e) {
    if (e.target && e.target.classList.contains('js-color-picker-input')) {
        const color = e.target.value;
        const toolbar = e.target.closest('markdown-toolbar');
        let textarea;
        if (toolbar && toolbar.getAttribute('for')) {
            textarea = document.getElementById(toolbar.getAttribute('for'));
        } else {
            textarea = e.target.closest('.ui.form, .comment-content').querySelector('textarea');
        }
        if (!textarea) return;
...
function watchAnnouncementsRepos() {
  ...
  if (isAnnouncementPage) {
    const unwatchFormButtonElement = document.querySelector('.octicon-eye-closed');
    const watchFormButtonElement = document.querySelector('#repo-header-watch button');

    if (unwatchFormButtonElement) {
      watchFormButtonElement.click();
      document.querySelector(`[data-fetch-url="${announcementPath}/action/watch"]`).click();
    }
    watchFormButtonElement.disabled = true;
  }
}
...
function relationPRtoIssuesTitle() {
  ...
  const branchNameAndRepoAndIDMatch = /^([^_]+).*[_]([^_]+\/[^_#\d]+)[_#]?(\d+)$/.exec(branchNameWithID);
  const branchNameAndIDMatch = /^([^_]+).*[_#](\d+)$/.exec(branchNameWithID);
  const issueRepoAndIDMatch = /([[^_#\d]+])[_#]?(\d+)$/.exec(branchNameWithID);
  const issueIDOnlyMatch = /(\d+)$/.exec(branchNameWithID);
  ...
  const fetchIssueDetails = async (mergeReposName, mergeIssueID) => {
    ...
    const commitSummary = Array.from(document.querySelectorAll('.commit-summary'), (v) => v.textContent.trim()).reverse();
...
const loadResource = (url, callback) => {
  ...
  element.setAttribute('nonce', document.querySelector('script').nonce);
  if (type === 'css') {
...
function reportGenerator() {
  ...
  loadResource('https://cdn.jsdelivr.net/npm/vue@3/dist/vue.global.prod.js', () => {
    ...
    const columns = [
      ...
      {name: 'Comments', sort: true, hidden: true, attributes: {style: 'width:200px', class: 'gridjs-td ellipsis'}}
    ];
    ...
    function commentsToChild() {
      ...
      for (const row of tbody) {
        ...
        if (subComment.textContent && !subComment.textContent.startsWith('  - ')) {
    ...
    function addExportButton(endDate) {
      ...
      copyBtn.addEventListener('click', async () => {
        ...
        copyBtn.setCustomValidity(copyToClipboard(item) ? window.config.i18n.copy_success : window.config.i18n.copy_error);
        copyBtn.reportValidity();
      });

      if (document.querySelector('#toggle-column-comments')?.checked) {
        if (!window.originalData.length) return;
    ...
    window.app = createApp({
      ...
      watch: {
        ...
        startDate(newValue) {
          ...
          if (this.quickday !== 1) this.endDate = newDay.toISOString().slice(0, 10);
        ...
        endDate(newValue) {
          ...
          if (this.quickday === 1) this.startDate = this.endDate;
          else this.startDate = newDay.toISOString().slice(0, 10);
        },
        orig(newValue) {
      ...
      methods: {
        async dateCtrl(day) {
          ...
          if (day >= 0) {
            ...
            if (this.quickday === 1) this.endDate = daysBefore(1);
            await this.$nextTick();
            this.queryReport();
        ...
        async queryReport(isNotOKButton) {
          ...
          const startDate = `${this.startDate}T00:00:00`;
          const startDateTime = new Date(startDate).getTime();
          const endDate = `${this.endDate}T23:59:59`;
          const endDateTime = new Date(endDate).getTime();

          const isSelf = this.userTitle === window.userTitle;
          const params = new URLSearchParams();
          params.set('state', this.state);
          params.set('since', `${startDate}+08:00`);
          params.set('before', `${endDate}+08:00`);
          params.set('page', 1);
          params.set('limit', 10000);
          if (this.by === 'created_by') params.set('by_created', 'true');
          else if (this.by === 'myself') params.set('myself', 'true');
          ...
          if (this.by === 'myself') {
            ...
            for (const repo of reposLists) {
              const commIndexKey = `${repo}_${indexKey}`;
              ...
              if (!cachedData) {
                ...
                cachedData = await get(`/api/v1/repos/${repo}/issues/comments?${indexKey}`);
          ...
          const data = [];
          const assigneeLists = [{full_name: window.dept, avatar_url: 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'}];
          for (const issue of resData || []) {
            ...
            if ((this.userTitle.length && (this.by === 'assigned_by' && !issue.assignees?.some((a) => a.full_name.startsWith(this.userTitle))) ||
              ...
              (this.userTitle.length && this.by === 'myself' && !issue.user.full_name.startsWith(this.userTitle) && commData.find((c) => c.user.full_name.startsWith(this.userTitle) && issue.html_url === c.issue_url) === undefined)) {
              continue;
            }
            ...
            if (commData.length) {
              ...
              if (commentsMatchingIssue.length > 0) {
                ...
                for (const comment of commentsMatchingIssue) {
                  comment.body = comment.body.replaceAll('\r', '');
                  const commentSignatureIndex = comment.body.lastIndexOf(this.userSignature[this.username]);
                  const cutLength = commentSignatureIndex === -1 || commentSignatureIndex > maxCommentLength ? maxCommentLength : commentSignatureIndex;
                  const commentText = cleanCellText(comment.body.slice(0, cutLength)).trim();
                  const updated_at = new Date(comment.updated_at).toLocaleString();
                  if (commentText) commentsContent += `&nbsp;&nbsp;- <a href="${comment.html_url}" data-tooltip-content="${updated_at}" target="_blank">${commentText}</a>` + ((commentSignatureIndex == -1 && comment.body.length > cutLength) || (commentSignatureIndex != -1 && comment.body.replace(this.userSignature[this.username], '').trim().length > cutLength) ? '...' : '') +'<br>';
                }
                ...
                if (issueTime >= startDateTime && issueTime <= endDateTime) {
                  issue.body = issue.body.replaceAll('\r', '');
                  const commentSignatureIndex = issue.body.lastIndexOf(this.userSignature[this.username]);
                  const cutLength = commentSignatureIndex === -1 || commentSignatureIndex > maxCommentLength ? maxCommentLength : commentSignatureIndex;
                  const commentText = cleanCellText(issue.body.slice(0, cutLength)).trim();
                  const created_at = new Date(issue.created_at).toLocaleString();
                  if (commentText) commentsContent += `&nbsp;&nbsp;- <a href="${issue.html_url}" data-tooltip-content="${created_at}" target="_blank">${commentText}</a>` + ((commentSignatureIndex == -1 && issue.body.length > cutLength) || (commentSignatureIndex != -1 && issue.body.replace(this.userSignature[this.username], '').trim().length > cutLength) ? '...' : '');
                  issue.state === 'open' ? reposSummaryData[repoName].open++ : reposSummaryData[repoName].closed++;
                } else {
              ...
              if (commentsContent) {
                data[data.length - 1].push(gridjs.html(commentsContent));
              }
            } else {
              const commentSignatureIndex = issue.body.replaceAll('\r', '').lastIndexOf(this.userSignature[this.username]);
              const cutLength = commentSignatureIndex === -1 || commentSignatureIndex > maxCommentLength ? maxCommentLength : commentSignatureIndex;
              data[data.length - 1].push(gridjs.html(cleanCellText(issue.body.slice(0, cutLength)) + (issue.body.length > cutLength || commentSignatureIndex > cutLength ? '...' : '')));
              onlyName && (this.isSelected[onlyName] = false);
              usersSummary(usersSummaryData, {full_name: onlyName, username: issue.user.username}, assigneeNames, heatmapData, issue.user.username);
              issue.state === 'open' ? reposSummaryData[repoName].open++ : reposSummaryData[repoName].closed++;
            }
        ...
        async selectAssignee(assignee, username, e) {
          if (!e.isTrusted) return;
          this.userTitle = assignee;
          this.username = username;
          if (username && !this.userSignature[username]) {
            const userInfo = await get(`/api/v1/users/${username}`);
            this.userSignature[username] = userInfo.description.replaceAll('\r', '');
          }
          this.queryReport();
      ...
      mounted() {
        ...
        const userSignatureFromLocalStorage = localStorage.getItem('user-signature')?.trim();
        this.userSignature = userSignatureFromLocalStorage ? {[this.username]: userSignatureFromLocalStorage} : {};
...
function addPageControlButton() {
  if (localStorage.getItem('top-pagination')) {
...
function addViewLogsButton(firstElement) {
  ...
  firstElement.append(viewLogButton);
}

function addRelatedButton(firstElement) {
  ...
  viewRelatedButton.addEventListener('click', () => {
    ...
    for (const el of relatedItems) {
      ...
      firstElement.insertAdjacentElement('afterend', parentElement);
    }
  });
  firstElement.append(viewRelatedButton);
}

function addReverseButton(firstElement) {
  ...
  reverseButtonElement.disabled = !timelineElement.querySelectorAll('.comment:not(.comment.issue-content-comment,.form)').length;
  const toggleReverse = () => {
  ...
  if (localStorage.getItem('timeline-reverse')) toggleReverse();
}

function addCreatedByListButton(firstElement) {
  function getTaskLists() {
    let taskLists;
    try {
      taskLists = document.querySelectorAll('.comment.issue-content-comment .task-list-item:not(:has(input:checked)) a');
    } catch (e) {
      taskLists = Array.from(document.querySelectorAll('.comment.issue-content-comment .task-list-item input:not(:checked)')).map(input => input.closest('.task-list-item').querySelector('a')).filter(Boolean);
    }
    return taskLists;
  }
  ...
  const generateTaskButtonElement = document.createElement('button');
  generateTaskButtonElement.className = 'ui button tiny tw-mt-1';
  generateTaskButtonElement.style.minHeight = 'unset';
  generateTaskButtonElement.textContent = 'Copy tasks';

  const getTaskList = async () => {

    generateTaskButtonElement.disabled = true;
    const createdByLists = {};
    const taskLists = getTaskLists();

    for (const taskList of taskLists) {
      const url = new URL(taskList.href);
      url.hash = '';
      const taskListInfo = await get(`${url.toString()}/info`);
      const createdBy = taskListInfo.convertedIssue.user.username;
      if (!createdByLists[createdBy]) {
        createdByLists[createdBy] = [];
      }
      createdByLists[createdBy].push(taskListInfo.convertedIssue);
    }

    const sortedByUserLists = Object.keys(createdByLists).sort((a, b) => { return createdByLists[a][0].user.full_name.localeCompare(createdByLists[b][0].user.full_name) });
    const assigneeUsers = sortedByUserLists.join(' @');
    let tasks = `Hi @${assigneeUsers}:\n`;
    for (const createdBy of sortedByUserLists) {
      const issues = createdByLists[createdBy];
      const fullName = issues[0].user.full_name ? ` (${issues[0].user.full_name})` : '';

      let i = 1;
      tasks += `\n@${createdBy}${fullName}:\n${issues.map(issue => {
        let labelSpan = '';
        const bugLabel = issue.labels.find(label => label.name.toLowerCase() === 'bug');
        if (bugLabel) {
          labelSpan = `<span style="background-color:#${bugLabel.color}">${bugLabel.name}</span>`;
        }
        return `${i++}. ${issue.repository.full_name}#${issue.number} ${issue.title} ${labelSpan}`;
      }).join('\n')}\n`;
    }

    generateTaskButtonElement.disabled = !sortedByUserLists.length;

    if (sortedByUserLists.length) {
      generateTaskButtonElement.setCustomValidity(copyToClipboard(tasks) ? window.config.i18n.copy_success : window.config.i18n.copy_error);
      generateTaskButtonElement.reportValidity();
    }
  };

  generateTaskButtonElement.addEventListener('click', getTaskList);
  firstElement.append(generateTaskButtonElement);
}
...
function copyToClipboard(text) {
  ...
  return tryCopyToClipboard(text);
}
  ...
  if (!document.querySelector('.copy-title-list')) {
    ...
    copyTitleButton.addEventListener('click', () => {
      ...
      for (const el of selectedIssuesLists) {
        const titleElement = el.closest('div.item').querySelector('a');
...
async function autoRemoveFiles() {
  ...
  for (const el of userComments) {
    ...
    if (updateDataSet) {
      const attachmentsElements = commentElement.querySelectorAll('.dropzone-attachments div a strong');
      const attachmentsSizes = Array.from(commentElement.querySelectorAll('.dropzone-attachments div div:nth-of-type(2)')).map((el) => parseSizeToBytes(el.textContent));
      let html;
      for (const attachEl of attachmentsElements) {
        ...
        const rawContent = commentID === 'content' ? commentElement.querySelector(`#${commentElement.id}-raw`) : commentElement.querySelector(`#issuecomment-${commentID}-raw`);
  ...
  if (lists.length && (isPRMerged || (doDel = confirm(`Are you sure to auto remove ${lists.length} file${lists.length > 1 ? 's' : ''}?\n\n${lists.map((list) => list.name).join('\n')}`)))) {
    for (const list of lists) {
      const contentElement =  list.raw.parentElement.querySelector('.render-content');
...
function addIssuesCheckBox() {
  ...
  const wrapper = document.querySelectorAll('#issue-list .item-main');

  for (const el of wrapper) {
    const index = el.querySelector('a.index')?.textContent.trim();
    ...
    el.querySelector('a').prepend(checkbox);
  }
}
...
function addCopySelfActivitesButton() {
  ...
  button.addEventListener('click', () => {
    const items = activity.querySelectorAll('.item-main');
    const fullTitleLists = [];
    const htmlItems = [];
    let i = 1;
    for (const el of items) {
      ...
      if (!id || !title || fullTitleLists.includes(title)) continue;
...
function addRemoveUserInTeamsButton() {
  if (location.pathname !== '/-/admin') return;

  const table = document.querySelector('.admin-setting-content table');
  if (!table) return;

  const newTr = table.querySelector('tr:last-child').cloneNode(true);
  newTr.querySelector('td').textContent = 'Remove user in all teams';
  const button = newTr.querySelector('button');
  button.type = 'button';
  button.addEventListener('click', async () => {
    const users = await get('/api/v1/admin/users?is_active=false&is_restricted=true&is_prohibit_login=true');
    const results = [];
    for (const user of users) {
      const joinOrgs = await get(`/api/v1/users/${user.username}/orgs`);
      for (const org of joinOrgs) {
        await post(`/api/v1/orgs/${org.name}/members/${user.username}`, null, 'DELETE');
        results.push(`Removed ${user.username} from ${org.name}`);
      }
    }
    alert(results.join('\n'));
  });
...
function projectsImageToDialog() {
  if (!location.pathname.match(/\/projects\/\d+/)) return;
  const imsges = document.querySelectorAll('.card-attachment-images img');
  if (!imsges) return;
  const detailModal = document.querySelector('#img-modal');
  if (!detailModal) return;
  detailModal.addEventListener('click', () => {
    detailModal.classList.value = 'tw-hidden';
    document.body.style.overflow = '';
  });
  for (const img of imsges) {
    img.addEventListener('click', () => {
      detailModal.innerHTML = img.outerHTML.replace(/width=".*?"/g, '').replace(/height=".*?"/g, '');
      detailModal.classList.value = 'ui dimmer active';
      document.body.style.overflow = 'hidden';
    });
  }
}

function imagesToModal() {
  if (!location.pathname.match(/\/(?:issues|pulls)\/\d+/)) return;
  const images = document.querySelectorAll('.comment-body a > img');
  if (!images) return;
  const detailModal = document.querySelector('#img-modal');
  if (!detailModal) return;
  detailModal.addEventListener('click', () => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('reportBtn')?.addEventListener('click', reportGenerator);
  ...
  const firstElement = document.querySelector('.comment.issue-content-comment');
  if (firstElement) {
    addViewLogsButton(firstElement);
    addRelatedButton(firstElement);
    addReverseButton(firstElement);
    addCreatedByListButton(firstElement);
  }
  addCopyTitleButton();
  ...
  addRemoveUserInTeamsButton();
  projectsImageToDialog();
  imagesToModal();
});
