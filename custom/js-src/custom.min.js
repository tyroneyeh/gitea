    ...
    window.app = createApp({
      ...
      methods: {
        ...
        filterRepos(repo, e) {
          ...
          if (target) target.classList.add('border');
        ...
        filterLabel(label, e) {
          const target = e?.currentTarget;
          if (target) target.classList.add('border');
          const newData = window.grid.config.data.filter(row => row[5].props.content.includes(`"${label}"`));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
          addExportButton(this.endDate);
        },
        filterUser(createdUser, assignedUser, e) {
          const target = e?.currentTarget;
          if (target) target.classList.add('border');
