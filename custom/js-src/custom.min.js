    ...
    window.app = createApp({
      ...
      methods: {
        ...
        filterRepos(repo, state) {
          ...
          const newData = !isDeselect ? window.originalData : window.grid.config.data.filter(row => row[3].props.content.replace(/<[^>]+>/g, '').includes(`${repo}#`) && row[6] === (state ? 'open' : 'closed'));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
