            ...
            if (commData.length) {
              ...
              if (commentsMatchingIssue.length > 0) {
                ...
                for (const comment of commentsMatchingIssue) {
                  ...
                  const commentText = cleanCellText(comment.body.slice(0, commentSignatureIndex === -1 ? maxCommentLength : commentSignatureIndex)).trim();
                  const updated_at = new Date(comment.updated_at).toLocaleString();
                  if (commentText) commentsContent += `&nbsp;&nbsp;- <a href="${comment.html_url}" data-tooltip-content="${updated_at}" target="_blank">${commentText}</a>` + (commentText.length > maxCommentLength ? '...' : '') +'<br>';
                }
