  ...
  loadResource('https://cdn.jsdelivr.net/npm/vue@3/dist/vue.global.prod.js', () => {
    ...
    window.app = createApp({
      ...
      methods: {
        ...
        filterRepos(repo) {
          const isDeselect = !this.isSelected[repo];
          !isDeselect && resetObjToFalse(this.isSelected);
          this.isSelected[repo] = isDeselect;
          const newData = !isDeselect ? window.originalData : window.grid.config.data.filter(row => row[3].props.content.replace(/<[^>]+>/g, '').includes(`${repo}#`));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
        ...
        filterLabel(label) {
          ...
          const newData = !isDeselect ? window.originalData : window.grid.config.data.filter(row => row[5].props.content.includes(`"${label}"`));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
          addExportButton(this.endDate);
        ...
        filterUser(createdUser, assignedUser) {
          ...
          const newData = !isDeselect ? window.originalData : window.grid.config.data.filter(row => createdUser && row[1].includes(createdUser) || assignedUser && row[2]?.includes(assignedUser));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
          addExportButton(this.endDate);
        ...
        async queryReport(e) {
          ...
          for (const issue of resData || []) {
            ...
            this.isSelected[onlyName] = false;
            ...
            for (const assignee of assigneeNames) {
              ...
              usersSummaryData[onlyAssigneeName] = usersSummaryData[onlyAssigneeName] || {created: 0, assigned: 0};
