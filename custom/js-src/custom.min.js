...
function autoSubscribe() {
  ...
  if (muteButton) {
    ...
    if (assigneeHasSelf) {
      post(form.action, {watch: 1}).then(res => res.text().then(data => {
        form.innerHTML = data;
      }));
      return;
    }
...
function waitForTextareaToBeReady() {
  ...
  const intervalId = setInterval(() => {
    ...
    const markdownEditor = document.querySelector('.combo-markdown-editor');
    attemptCount++;
        ...
        async queryReport(e) {
          ...
          for (const issue of resData || []) {
            ...
            if (this.by === 'myself') {
              ...
              if (comm.length) {
                data[data.length - 1].push(comm[0].body);
              }
            }
          }
