      ...
      methods: {
        ...
        async queryReport(isNotOKButton) {
          ...
          if (this.by === 'myself') {
            ...
            commParams.set('since', `${startDate}+08:00`);
            commParams.set('before', `${endDate}+08:00`);
            ...
            if (commData.length) {
              ...
              if (commentsMatchingIssue.length > 0) {
                ...
                for (const comment of commentsMatchingIssue) {
                  const commentBody = cleanCellText(comment.body.slice(0, maxCommentLength));
                  const commentSignatureIndex = commentBody.lastIndexOf(userSignature);
                  const commentText = commentBody.slice(0, commentSignatureIndex === -1 ? maxCommentLength : commentSignatureIndex)?.trim();
                  const updated_at = new Date(comment.updated_at).toLocaleString();
                  commentsContent += `&nbsp;&nbsp;- <a href="${comment.html_url}" data-tooltip-content="${updated_at}" target="_blank">${commentText}</a>` + (comment.body.length > maxCommentLength ? '...' : '') +'<br>';
                }
