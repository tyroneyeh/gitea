...
function addCopyTitleButton() {
  ...
  copyTitleButton.addEventListener('click', () => {
    ...
    const html = `<a href="${window.location.href}">${id}</a>\t${title}`;
    const plain = `${id}\t${title}`;
    const copyItem = new ClipboardItem({'text/html': new Blob([html], {type: 'text/html'}), 'text/plain': new Blob([plain], {type: 'text/plain'})});
    copyTitleButton.setCustomValidity(copyToClipboard(copyItem) ? window.config.i18n.copy_success : window.config.i18n.copy_error);
