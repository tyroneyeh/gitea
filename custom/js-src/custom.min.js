    ...
    window.app = createApp({
      ...
      methods: {
        ...
        filterRepos(repo, state, e) {
          ...
          const newData = !isDeselect ? window.originalData : e.altKey ? window.grid.config.data.filter(row => !row[3].props.content.replace(/<[^>]+>/g, '').includes(`${repo}#`)) : window.grid.config.data.filter(row => row[3].props.content.replace(/<[^>]+>/g, '').includes(`${repo}#`) && row[6] === (state ? 'open' : 'closed'));
          window.grid.updateConfig({
            data: newData,
          }).forceRender();
