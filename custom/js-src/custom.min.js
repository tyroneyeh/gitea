            ...
            if (commData.length) {
              ...
              if (commentsMatchingIssue.length > 0) {
                ...
                for (const comment of commentsMatchingIssue) {
                  const commentSignatureIndex = comment.body.replaceAll('\r', '').lastIndexOf(this.userSignature[this.username]);
                  ...
                  if (commentText) commentsContent += `&nbsp;&nbsp;- <a href="${comment.html_url}" data-tooltip-content="${updated_at}" target="_blank">${commentText}</a>` + (comment.body.length > cutLength || commentSignatureIndex != -1 ? '...' : '') +'<br>';
                }
