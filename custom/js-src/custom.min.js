    ...
    for (const entry of entries) {
      if (entry.isIntersecting && !window.isProgrammaticScroll) {
        ...
        if (eyes) {
          ...
          setTimeout(() => post(`${commentHeader.querySelector('.select-reaction').dataset.actionUrl}/react`, {content: 'eyes'}), reactionDelay);
        }
      }
    }
  }, {
...
function reportGenerator() {
  ...
  loadResource('https://cdn.jsdelivr.net/npm/vue@3/dist/vue.global.prod.js', () => {
    ...
    const userSignature = localStorage.getItem('user-signature')?.trim();
    ...
    const columns = [
      ...
      {name: 'Type', sort: true, hidden: true, attributes: {style: 'white-space:nowrap;overflow:hidden'}},
      {name: 'Creator', sort: true, hidden: true, attributes: {style: 'white-space:nowrap;overflow:hidden'}},
      {name: 'Assignee', sort: true, hidden: true, attributes: {style: 'white-space:nowrap;overflow:hidden'}},
      {name: 'Repos.#ID', sort: {compare: (a, b) => a.props.content.replace(/<[^>]*>?/gm, '').localeCompare(b.props.content.replace(/<[^>]*>?/gm, ''))}, attributes: {style: 'width:auto;white-space:nowrap;overflow:hidden'}},
      {name: 'Title', sort: true, attributes: {style: 'width:auto'}},
      {name: 'Labels', sort: true, hidden: true, attributes: {style: 'white-space:nowrap;overflow:hidden'}},
      {name: 'Status', sort: true, hidden: true, attributes: {style: 'width:80px'}},
      {name: 'Created At', sort: true, hidden: true, attributes: {style: 'width:auto;white-space:nowrap;overflow:hidden'}},
      {name: 'Updated At', sort: true, hidden: true, attributes: {style: 'width:auto;white-space:nowrap;overflow:hidden'}},
      {name: 'PCT', sort: true, hidden: true, attributes: {style: 'width:70px;white-space:nowrap'}},
      {name: 'Due Date', sort: true, hidden: true, attributes: {style: 'width:auto;white-space:nowrap;overflow:hidden'}},
    ];
    ...
    window.app = createApp({
      ...
      data() {
        return {
          ...
          labels: localStorage.getItem('report-labels') || '',
      ...
      watch: {
        ...
        startDate(newValue) {
          ...
          if (new Date(this.startDate).getTime() < new Date(this.endDate).getTime() || newDay.getTime() > Date.now()) return;
          this.endDate = newDay.toISOString().slice(0, 10);
        },
        endDate(newValue) {
          const newDay = new Date(newValue);
          if (new Date(this.startDate).getTime() < new Date(this.endDate).getTime()) return;
          newDay.setDate(newDay.getDate() - 7);
        ...
        labels(newValue) {
          if (newValue === '') localStorage.removeItem('report-labels');
          else localStorage.setItem('report-labels', newValue);
        },
        q(newValue) {
      ...
      methods: {
        ...
        addExportButton() {
          ...
          copyBtn.addEventListener('click', async () => {
            ...
            const item = new ClipboardItem({
              'text/html': new Blob([document.querySelector('.gridjs-table').outerHTML], { type: 'text/html' }),
          ...
          if (this.by === 'myself' && document.querySelector('#toggle-column-Comments').checked) {
            const commToChildBtn = document.createElement('button');
            commToChildBtn.textContent = 'Comment to Sub';
            commToChildBtn.className = 'ui button';
            search.append(commToChildBtn);
            commToChildBtn.addEventListener('click', async () => {
              const tbody = window.grid.config.container.querySelectorAll('.gridjs-tbody tr');
              document.querySelector('[data-column-id="comments"]').remove();
              for (const row of tbody) {
                const comment = row.querySelector('td[data-column-id="comments"]');
                if (comment) {
                  const newTr = document.createElement('tr');
                  comment.colSpan = row.querySelectorAll('td').length - 1;
                  comment.innerHTML = `<span style="white-space:pre;text-wrap:auto">\t- ${comment.innerHTML}</span>`;
                  newTr.append(comment);
                  row.after(newTr);
                }
              }
              commToChildBtn.remove();
            });
          }
        },
        titleControl() {
          ...
          for (const column of columns) {
            if (localStorage.getItem(`report-column-${column.name}`)) {
              column.hidden = false;
            }
            const checkbox = document.createElement('input');
            checkbox.type = 'checkbox';
            checkbox.className = 'tw-align-middle tw-mr-1';
            checkbox.id = `toggle-column-${column.name}`;
            checkbox.checked = !column.hidden;
            checkbox.addEventListener('change', (event) => {
              column.hidden = !event.target.checked;
              if (event.target.checked) localStorage.setItem(`report-column-${column.name}`, '1');
              else localStorage.removeItem(`report-column-${column.name}`);
            ...
            label.append(document.createTextNode(column.name));
        ...
        async queryReport(e) {
          ...
          if (target) e.target.disabled = true;
          setCursor('wait');
          ...
          if (this.by === 'myself') {
            ...
            for (const issue of resData || []) {
              if (!reposLists.includes(issue.repository.full_name)) {
                reposLists.push(issue.repository.full_name);
              }
            }

            const commParams = new URLSearchParams();
            commParams.set('since', `${this.startDate}T00:00:00Z`);
            commParams.set('before', `${this.endDate}T23:59:59Z`);
            ...
            for (const repo of reposLists) {
              ...
              if (!cachedData) {
                ...
                db.set(commIndexKey, commData);
              }
              commData.push(...cachedData);
            }
            ...
            if (!commentsColumn) {
              columns.push({
                name: 'Comments',
                sort: true,  hidden: true, attributes: {style: 'width:auto'}
              });
            }
          ...
          for (const issue of resData || []) {
            if (this.by === 'assigned_by' && issue.assignees) {
              ...
              const startAt = new Date(`${this.startDate}T00:00:00Z`).getTime();
              const endAt = new Date(`${this.endDate}T23:59:59Z`).getTime();
            ...
            for (let j = 0; j < labels.length; j++) {
              ...
              label += `<span style="background-color:#${labels[j].color}" class="ui label">${labels[j].name}</span> `;
            }
            ...
            const issueID = `<a href="${issue.html_url}" target="_blank" rel="noopener">${issue.repository.full_name}#${issue.number}</a>`;

            // eslint-disable-next-line no-undef
            data.push([issue.pull_request ? 'PR' : 'Issue', issue.user.full_name, issue.assignees?.map((a) => a.full_name).join(', '), gridjs.html(issueID), issue.title, gridjs.html(label), issue.state, issue.created_at.slice(0, 19).replace('T', ' '), issue.updated_at.slice(0, 19).replace('T', ' '), `${pct}%`, issue.due_date ? issue.due_date.slice(0, 10) : '']);
            if (this.by === 'myself') {
              const comm = commData.filter((c) => c.issue_url === issue.html_url);
              const lastCommentBody = comm.length ? comm[comm.length - 1].body : '';
              const commentSignatureIndex = lastCommentBody.indexOf(userSignature);
              const commentText = commentSignatureIndex === -1 ? lastCommentBody : lastCommentBody.slice(0, commentSignatureIndex);
              data[data.length - 1].push(commentText.trim().slice(0, 100) + (commentText.length > 100 ? '...' : ''));
            }
          }
          ...
          // data.sort((a, b) => a[3].props.content.replace(/<[^>]*>?/gm, '').localeCompare(b[3].props.content.replace(/<[^>]*>?/gm, '')));
          ...
          if (target) e.target.disabled = false;
          setCursor('');

        },
        selectAssignee(assignee, username) {
          this.userTitle = assignee;
          this.username = username;
          localStorage.setItem('report-username', username);
          localStorage.setItem('report-userTitle', assignee);
        },
      },
      mounted() {
      ...
      methods: {
        async queryRelease() {
          ...
          if (!data || !data.length) {
            ...
            data.length && db.set(reposName, data);
          }
...
function addViewLogsButton() {
  ...
  if (!events) return;
  if (localStorage.getItem('hide-logs')) {
  ...
  viewLogButton.addEventListener('click', () => {
    localStorage.getItem('hide-logs') ? localStorage.removeItem('hide-logs') : localStorage.setItem('hide-logs', '1');
    for (const el of events) el.classList.toggle('tw-hidden');
  });
  ...
  if (!document.querySelector('.copy-title-list')) {
    ...
    copyTitleButton.addEventListener('click', () => {
      ...
      for (const el of selectedIssuesLists) {
        ...
        htmlItems.push(`${i++}.<span style="white-space:pre">\t<a href="${href}">${id}</a>\t${escapeHtml(title)}</span>`);
      }
...
function addCommitsCheckBox() {
  ...
  for (const el of wrapper) {
    ...
    const checkbox = document.createElement('input');
    checkbox.type = 'checkbox';
    checkbox.title = 'Select to copy issue titles (Shift+Click to select all)';
...
async function autoRemoveFiles() {
  ...
  if (sessionStorage.getItem(skipKey) || !Array.from(document.querySelectorAll('.list.labels-list a')).some((el) => el.textContent.toLowerCase() === 'solved')) return;
  const lists = [];
  ...
  for (const el of userComments) {
    ...
    if (updateDataSet) {
      ...
      for (const attachEl of attachmentsElements) {
        ...
        const fileName = attachEl.textContent;
        ...
        rawContent.textContent = rawContent.textContent.replace(new RegExp(`\\[${escapeRegExp(fileName)}\\]\\([^)]*\\)`, 'g'), `~~Auto remove [${fileName}]~~`);
        html = html || contentElement.innerHTML;
        html = html.replace(new RegExp(`<a .*${escapeRegExp(fileName)}</a>`, 'g'), `<del>Auto remove [${fileName}]</del>`);
        lists.push({
          name: fileName,
  ...
  let doDel = true;
  if (lists.length && (doDel = confirm(`Are you sure to auto remove ${lists.length} file${lists.length > 1 ? 's' : ''}?\n\n${lists.map((list) => list.name).join('\n')}`))) {
...
function addIssuesCheckBox() {
  ...
  for (const el of wrapper) {
    ...
    checkbox.type = 'checkbox';
    checkbox.title = 'Select to copy issue titles (Shift+Click to select all)';
    checkbox.className = 'issue-checkbox tw-mr-1 tw-align-middle';
    ...
    checkbox.addEventListener('click', (e) => {
      ...
      for (const el of selectedCommits) {
        ...
        htmlItems.push(`${i++}.<span style="white-space:pre">\t<a href="${el.dataset.link}">${el.value}</a>\t${el.dataset.title}</span>`);
      }
...
function addCopySelfActivitesButton() {
  ...
  button.addEventListener('click', () => {
    ...
    for (const el of items) {
      ...
      htmlItems.push(`${i++}.<span style="white-space:pre">\t<a href="${idElement.href}">${id}</a>\t${escapeHtml(title)}</span>`);
    }
    const data = new ClipboardItem({'text/html': new Blob([htmlItems.join('<br>\n')], {type: 'text/html'}), 'text/plain': new Blob([fullTitleLists.join('\n')], {type: 'text/plain'})});
