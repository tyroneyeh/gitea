...
function reportGenerator() {
  ...
  loadResource('https://cdn.jsdelivr.net/npm/vue@3/dist/vue.global.prod.js', () => {
    ...
    window.app = createApp({
      ...
      methods: {
        ...
        async queryReport(e) {
          ...
          if (this.by === 'myself') {
            ...
            commData = commData.filter((v) => v.user.full_name === this.userTitle);
          }
          ...
          for (const issue of resData) {
            ...
            if ((this.userTitle.length && (this.by === 'assigned_by' && !issue.assignees?.some((a) => a.full_name.startsWith(this.userTitle))) ||
              ...
              (this.userTitle.length && this.by === 'myself' && commData.find((c) => this.userTitle === c.user.full_name && issue.html_url === c.issue_url) === undefined)) {
              continue;
            }
            ...
            // eslint-disable-next-line no-undef
            data.push([`${i++}.`, issue.pull_request ? 'PR' : 'Issue', issue.user.full_name, issue.assignees?.map((a) => a.full_name).join(', '), gridjs.html(issueID), issue.title, gridjs.html(label), issue.state, issue.created_at.slice(0, 19).replace('T', ' '), issue.updated_at.slice(0, 19).replace('T', ' '), `${pct}%`, issue.due_date ? issue.due_date.slice(0, 10) : '']);
          }
          ...
          if (this.userTitle !== window.userTitle) {
            localStorage.setItem('report-username', this.userTitle);
          } else {
      ...
      mounted() {
        const savedUsername = localStorage.getItem('report-username');
        if (savedUsername) {
          this.userTitle = savedUsername;
        }
...
function addRevisionNumber() {
  ...
  for (const el of commitIDElemtns) {
    if (document.querySelector('[name=q]').value.trim() === "") el.prepend(`r${revision--} `);
    const copyBtn = document.createElement('button');
