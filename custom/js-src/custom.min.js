      ...
      methods: {
        ...
        async queryReport(e) {
          ...
          for (const issue of resData || []) {
            ...
            const onlyName = issue.user.full_name.split(' ').slice(0, 2).join(' ');
            usersSummaryData[onlyName] = usersSummaryData[onlyName] || {created: 0, assigned: 0};
            const assigneeNames = issue.assignees?.map((a) => a.full_name) || [];
            for (const assignee of assigneeNames) {
              const onlyAssigneeName = assignee.split(' ').slice(0, 2).join(' ');
              usersSummaryData[onlyAssigneeName] = usersSummaryData[assignee] || {created: 0, assigned: 0};
              usersSummaryData[onlyAssigneeName].assigned++;
            }
            usersSummaryData[onlyName].created++;
            ...
            // eslint-disable-next-line no-undef
            data.push([issue.pull_request ? 'PR' : 'Issues', onlyName, issue.assignees?.map((a) => a.full_name.split(' ').slice(0, 2).join(' ')).join(', '), gridjs.html(issueID), issue.title, gridjs.html(label), issue.state, issue.created_at.slice(0, 19).replace('T', ' '), issue.updated_at.slice(0, 19).replace('T', ' '), `${pct}%`, issue.due_date ? issue.due_date.slice(0, 10) : '']);
            if (commData.length) {
              const maxLen = 100;
          ...
          else this.usersSummary = Object.fromEntries(Object.entries(usersSummaryData).sort((a, b) => b[1].created - a[1].created));
