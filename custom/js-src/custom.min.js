            ...
            if (commData.length) {
              ...
              if (commentsMatchingIssue.length > 0) {
                ...
                for (const comment of commentsMatchingIssue) {
                  ...
                  if (commentText) commentsContent += `&nbsp;&nbsp;- <a href="${comment.html_url}" data-tooltip-content="${updated_at}" target="_blank">${commentText}</a>` + ((commentSignatureIndex == -1 && cutLength > comment.body.length) || comment.body.replace(this.userSignature[this.username], '').trim().length > cutLength ? '...' : '') +'<br>';
                }
