    ...
    window.app = createApp({
      ...
      watch: {
        ...
        startDate(newValue) {
          ...
          if (eDate - sDate > 31536000000) this.startDate = new Date(eDate - 31536000000).toISOString().slice(0, 10);
        ...
        endDate(newValue) {
          ...
          const sDate = new Date(this.startDate).getTime();
          const eDate = new Date(this.endDate).getTime();
          if (eDate - sDate > 31536000000) this.endDate = new Date(sDate + 31536000000).toISOString().slice(0, 10);
          const newDay = new Date(newValue);
      ...
      methods: {
        ...
        onToggle(target, e) {
          if (target === 'repos') {
            ...
            this.isReposOpen ? localStorage.setItem('repos-details-open', e.target.open) : localStorage.removeItem('repos-details-open');
          }
          if (target === 'labels') {
            this.isLabelsOpen = e.target.open;
            this.isLabelsOpen ? localStorage.setItem('labels-details-open', e.target.open) : localStorage.removeItem('labels-details-open');
          }
          if (target === 'users') {
            this.isUsersOpen = e.target.open;
            this.isUsersOpen ? localStorage.setItem('users-details-open', e.target.open) : localStorage.removeItem('users-details-open');
          }
        },
        filterRepos(repo, state, e) {
        ...
        async queryReport(e) {
          ...
          if (!resData) {
            ...
            resData?.length && db.set(indexKey, resData);
          }
          ...
          if (this.by === 'myself') {
            ...
            for (const repo of reposLists) {
              ...
              if (!cachedData) {
                ...
                commData?.length && db.set(commIndexKey, commData);
              }
          ...
          for (const issue of resData || []) {
            ...
            for (const assignee of assigneeNames) {
              const onlyAssigneeName = assignee;
      ...
      mounted() {
        ...
        loadResource('https://cdn.jsdelivr.net/npm/gridjs/dist/gridjs.umd.js', async () => {
          ...
          this.isReposOpen = localStorage.getItem('repos-details-open');
          this.isLabelsOpen = localStorage.getItem('labels-details-open');
          this.isUsersOpen = localStorage.getItem('users-details-open');
