            ...
            if (commData.length) {
              ...
              if (commentsMatchingIssue.length > 0) {
                ...
                for (const comment of commentsMatchingIssue) {
                  const commentSignatureIndex = comment.body.replaceAll('\r', '').lastIndexOf(userSignature);
                  ...
                  if (commentText) commentsContent += `&nbsp;&nbsp;- <a href="${comment.html_url}" data-tooltip-content="${updated_at}" target="_blank">${commentText}</a>` + (commentSignatureIndex === -1 && comment.body.length > cutLength || commentSignatureIndex > cutLength ? '...' : '') +'<br>';
                }
