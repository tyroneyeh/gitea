...
function addCreatedByListButton(firstElement) {
  ...
  const getTaskList = async () => {
    ...
    for (const createdBy of sortedByUserLists) {
      ...
      const bugLabel = issues[0].labels.find(label => label.name.toLowerCase() === 'bug');
      let labelSpan = '';
      if (bugLabel) {
        labelSpan = `<span style="background-color:#${bugLabel.color}">${bugLabel.name}</span>`;
      }
      let i = 1;
      tasks += `\n@${createdBy}${fullName}:\n${issues.map(issue => `${i++}. ${issue.repository.full_name}#${issue.number} ${issue.title} ${labelSpan}`).join('\n')}\n`;
    }
