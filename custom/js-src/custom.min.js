...
function autoSaveDraft() {
  ...
  for (const textarea of document.querySelectorAll('textarea:not([class="tw-hidden"])')) {
    const form = textarea.closest('form');
    if (!form) break;
    const isEasyMDE = localStorage.getItem('markdown-editor-comment') === 'easymde';
    const signatureContent = document.querySelector('#signature')?.value;
...
function autoSubscribe() {
  ...
  if (muteButton) {
    const form = muteButton.closest('form');

    const isSubscribeOrUnsubscribe = ['#subscribe', '#unsubscribe'].includes(location.hash);
    if (isSubscribeOrUnsubscribe) {
      const subscribe = location.hash === '#subscribe' ? 1 : 0;
      post(form.action, {watch: subscribe}).then(res => res.text().then(data => {
        form.innerHTML = data;
      }));
    }

    const assigneeHasSelf = document.querySelector('.relaxed').querySelector(`span[title="${window.loginedUser}"]`);
    if (!assigneeHasSelf) return;
