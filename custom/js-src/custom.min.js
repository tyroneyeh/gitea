...
function autoSaveDraft() {
  ...
  for (const textarea of document.querySelectorAll('textarea')) {
    ...
    if (isEasyMDE) {
      textarea.closest('form').querySelector('.CodeMirror-lines')?.addEventListener('mouseleave', () => doSave('mouseleave'));
    } else {
    ...
    form.addEventListener('submit', async () => {
      if (!isSaved) return;
      isSaved = false;
      if (textarea.value === signatureContent) textarea.value = '';
        ...
        async queryReport() {
          ...
          if (window.firstreport) {
            ...
            for (const key of keysToDelete) {
              if (key.includes('assigned') || key.includes('created') || key.includes('mentioned')) await db.remove(key);
            }
          }
          ...
          for (const issue of resData) {
            ...
            // eslint-disable-next-line no-undef
            data.push([`${no}.`, issue.pull_request ? 'PR' : 'Issue', issue.user.full_name, issue.assignees?.map((a) => a.full_name).join(', '), gridjs.html(issueID), issue.title, gridjs.html(label), issue.state, issue.created_at.slice(0, 19).replace('T', ' '), issue.updated_at.slice(0, 19).replace('T', ' '), `${pct}%`, issue.due_date ? issue.due_date.slice(0, 10) : '']);
          }
...
function addCopyTitleButton() {
  ...
  copyTitleButton.textContent = window.config.i18n.copy_success.slice(0, 2);
  copyTitleButton.addEventListener('click', () => {
    ...
    const issueTitle = document.querySelector('.issue-title h1').textContent.trim().split('\n')[0];
    copyTitleButton.setCustomValidity(copyToClipboard(new ClipboardItem({'text/html': new Blob([`<a href="${window.location.href}">${id}</a>\t${issueTitle}`], {type: 'text/html'}), 'text/plain': new Blob([`${id}\t${issueTitle}`], {type: 'text/plain'})})) ? window.config.i18n.copy_success : window.config.i18n.copy_error);
...
function addIssuesCheckBox() {
  ...
  for (const el of wrapper) {
    ...
    checkbox.addEventListener('click', (e) => {
      ...
      let i = 1;
      for (const el of selectedCommits) {
        fullTitleLists.push(`${i++}. ${el.value}`);
      }
      if (fullTitleLists.length) copyToClipboard(fullTitleLists.join('\n'));
    });
...
function excludeSolvedIssues() {
  ...
  issueLink.href = issueLink.href.replace('assigned', 'assigned&q=-label:Solved');
}
...
function addCopySelfActivitesButton() {
  ...
  button.addEventListener('click', () => {
    ...
    for (const el of items) {
      ...
      htmlItems.push(`<a href="${idElement.href}">${id}</a>\t${title}`);
    }
