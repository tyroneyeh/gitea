...
function autoSaveDraft() {
  ...
  for (const textarea of document.querySelectorAll('textarea')) {
    ...
    const doSave = () => {
      ...
      if (content.length && content !== signatureContent) {
...
function relationPRtoIssuesTitle() {
  ...
  const branchNameAndRepoAndIDMatch = /^([^_]+)_.*_([^_]+)[_#](\d+)$/.exec(branchNameWithID);
  ...
  if (branchNameAndRepoAndIDMatch?.length > 3) {
    ...
    fetchIssueDetails(branchNameAndRepoAndIDMatch[2].includes('/') ? branchNameAndRepoAndIDMatch[2] : reposName, branchNameAndRepoAndIDMatch[3]);
