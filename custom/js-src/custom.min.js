...
function addCreatedByListButton(firstElement) {
  function getTaskLists() {
    let taskLists;
    try {
      taskLists = document.querySelectorAll('.first .task-list-item:not(:has(input:checked)) a');
    } catch (e) {
      taskLists = Array.from(document.querySelectorAll('.first .task-list-item input:not(:checked)')).map(input => input.closest('.task-list-item').querySelector('a')).filter(Boolean);
    }
    return taskLists;
  }
