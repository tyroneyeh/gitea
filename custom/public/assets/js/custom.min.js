...
function hiToAssignee(userNames) {
  ...
  for (const availableUserElement of availableUserElements) {
    ...
    } else if (availableUserElement.classList.contains('checked') !== isChecked) post(assigneeInput.closest('div').getAttribute('data-update-url'), {action, id});
  }
  if (assigneeFound) {
    assigneeInput.closest('div').querySelector('.dropdown').click();
  }
}
...
function autoLabels(content) {
  ...
  const labelElementDiv = labelElement.closest('div');
  const availableLabelElements = Array.from(labelElementDiv.querySelectorAll('.menu a.item:not(.checked)'));
  ...
  for (const [label, pattern] of Object.entries(_autoLabels)) {
    if (pattern.test(content)) {
      ...
      if (contentLabelElement) {
        ...
        post(labelElementDiv.getAttribute('data-update-url'), {action, id});
        break;
      }
    }
  }
  ...
  if (titleInputElement) {
    ...
    if (labelFound) {
      labelElementDiv.querySelector('.dropdown').click();
    }
  }
}
