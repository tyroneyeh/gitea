 ...
 */
function hiToAssignee(usernames, target) {
  ...
  const availableUsers = Array.from(assigneeElement.querySelectorAll('.dropdown a.item span.gt-ellipsis'), (el) => el.closest('a.item'));

  window.hiToAssigneeFound = false;
  for (const item of availableUsers) {
    const username = item.querySelector('.gt-ellipsis').title;
    window.hiToAssigneeFound = !item.classList.contains('checked');
    if (usernames.includes(username) === window.hiToAssigneeFound) {
  ...
  for (const [label, pattern] of Object.entries(_autoLabels)) {
    if (pattern.test(content)) {
      ...
      if (contentLabelElement) {
        contentLabelElement.closest('a').classList.add('checked');
        window.autoLabelsFound = true;
        break;
      }
    }
  }
  ...
  if (titleInputElement) {
    ...
    if (titleLabelMatch) {
      ...
      if (titleLabelElement) {
        titleLabelElement.closest('a').classList.add('checked');
      }
    }
    const deptLabelElement = availableLabelElements.find((el) => el.textContent === window.dept);
    if (deptLabelElement) {
      deptLabelElement.closest('a').classList.add('checked');
      window.autoLabelsFound = true;
    }
  }
}
...
function autoSaveDraft() {
  ...
  for (const textarea of document.querySelectorAll('textarea')) {
    ...
    form.addEventListener('submit', () => {
      ...
      if (window.autoLabelsFound) {
        const labelInput = document.querySelector('input[name="label_ids"]').closest('div').querySelector('.dropdown');
        labelInput.dispatchEvent(new Event('click', {bubbles: false}));
        labelInput.dispatchEvent(new Event('click', {bubbles: false}));
      }
      if (window.hiToAssigneeFound) {
        const assigneeInput = document.querySelector('input[name="assignee_ids"]').closest('div').querySelector('.dropdown');
        assigneeInput.dispatchEvent(new Event('click', {bubbles: false}));
        assigneeInput.dispatchEvent(new Event('click', {bubbles: false}));
      }
    });
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (pageControl) {
    document.querySelector('#issue-list')?.parentNode.insertBefore(pageControl.cloneNode(true), document.querySelector('#issue-list'));
  }
});
