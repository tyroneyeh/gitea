...
function hiToAssignee(userNames) {
  ...
  for (const availableUserElement of availableUserElements) {
    ...
    if (availableUserElement.classList.contains('checked') !== isChecked) post(assigneeInput.closest('div').getAttribute('data-update-url'), {action, id});
  }
}
...
function autoLabels(content) {
  ...
  const availableLabelElements = Array.from(labelElement.closest('div').querySelectorAll('.menu a.item:not(.checked)'));
  ...
  for (const [label, pattern] of Object.entries(_autoLabels)) {
    if (pattern.test(content)) {
      ...
      if (contentLabelElement) {
        ...
        post(labelElement.closest('div').getAttribute('data-update-url'), {action, id});
        break;
      }
    }
  }
