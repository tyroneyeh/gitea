class SimpleDB {
  constructor(storeName = 'items', lifeTime = 3600, dbName = 'gitea') {
    this.dbName = dbName;
  ...
  async init() {
    ...
    const finalVersion = await this._ensureStoreExists();

    this.db = await new Promise((resolve, reject) => {
      const req = indexedDB.open(this.dbName, finalVersion);
      req.onupgradeneeded = () => {
        const db = req.result;
  ...
  _cleanExpired(items, store) {
    if (!this.lifeTime || this.lifeTime <= 0) return items;
  ...
  async getAll() {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.onsuccess = () => {
        const items = this._cleanExpired(req.result, store);
        resolve(items);
      };
    ...
    window.app = createApp({
      ...
      methods: {
        async queryReport() {
          ...
          const db = new SimpleDB('reportissue');
