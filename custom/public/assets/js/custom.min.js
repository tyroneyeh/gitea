 ...
 */
async function hiToAssignee(userNames) {
  const assigneeInput = document.querySelector('input[name="assignee_ids"]');
  if (!assigneeInput) return;
  const assigneeElement = assigneeInput.closest('div');
  const availableUserElements = Array.from(assigneeElement.querySelectorAll('.dropdown a.item span.gt-ellipsis'), (el) => el.closest('a.item'));
  ...
  for (const availableUserElement of availableUserElements) {
    ...
    const id = availableUserElement.getAttribute('data-value');
    if (titleInputElement) {
      if (availableUserElement.classList.contains('checked') !== isChecked) {
        assigneeFound = true;
        availableUserElement.click();
      }
    } else if (availableUserElement.classList.contains('checked') !== isChecked) await post(assigneeElement.getAttribute('data-update-url'), {action, id});
  }
  if (assigneeFound) {
    assigneeElement.querySelector('.dropdown').click();
  }
}
...
async function autoLabels(content) {
  ...
  const availableLabelElements = Array.from(labelElement.querySelectorAll('.menu a.item:not(.checked)'));
  ...
  for (const [label, pattern] of Object.entries(_autoLabels)) {
    if (pattern.test(content)) {
      ...
      if (contentLabelElement) {
        contentLabelElement.click();
        const action = 'attach';
        const id = contentLabelElement.getAttribute('data-value');
        await post(labelElement.getAttribute('data-update-url'), {action, id});
        break;
      }
    }
  }
  ...
  if (titleInputElement) {
    ...
    if (labelFound) {
      labelElement.querySelector('.dropdown').click();
    }
  }
}
      ...
      methods: {
        async queryReport() {
          ...
          for (const issue of resData) {
            ...
            if ((this.username && (this.by === 'assigned_by' && !issue.assignees?.some((a) => a.username === this.username)) ||
              (this.by === 'created_by' && issue.user.username !== this.username)) ||
...
function addPageControlBar() {
  ...
  if (pageControlElement) {
    ...
    if (issueListElement) {
      issueListElement.parentNode.insertBefore(pageControlElement.cloneNode(true), issueListElement);
    }
  }
}

function addViewEventButton() {
  ...
  viewEventButton.className = 'ui button tw-mt-4 tw-ml-1';
  viewEventButton.textContent = `Logs (${events.length})`;
  viewEventButton.addEventListener('click', () => {
    for (const el of events) el.classList.toggle('tw-hidden');
  });
  first.append(viewEventButton);
}
...
function addRelatedButton() {
  ...
  const viewEventButton = document.createElement('button');
  viewEventButton.className = 'ui button tw-mt-4 tw-ml-1';
  viewEventButton.textContent = `Related (${related.length})`;
  viewEventButton.addEventListener('click', () => {
...
function AddReverseButton() {
  ...
  reverseButton.className = 'ui button tw-mt-4 tw-ml-1';
  reverseButton.textContent = 'Reverse';
  reverseButton.addEventListener('click', () => {
    const action = reverseButton.textContent.includes('Reverse') ? 'Obverse' : 'Reverse';
    reverseButton.textContent = action;
    for (const el of timeline) el.classList.toggle('timeline-reverse');
  });
...
function addCopyTitleButton() {
  ...
  copyTitleButton.className = 'ui button tw-mt-4 tw-ml-1';
  copyTitleButton.textContent = 'Copy title';
  copyTitleButton.addEventListener('click', () => {
    const id = document.querySelector('.octicon-copy').closest('button').getAttribute('data-clipboard-text');
    const title = document.querySelector('.issue-title h1').firstChild.textContent.trim();
    const fullTitle = `- [ ] #${id}\t${title}`;
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(fullTitle);
    } else {
      const textarea = document.createElement('textarea');
      textarea.value = fullTitle;
...
function addCopyTitleListButton() {
  ...
  if (!document.querySelector('.copy-title-list')) {
    ...
    copyTitleButton.textContent = 'Copy Lists';
    copyTitleButton.addEventListener('click', () => {
      ...
      for (const el of selectedIssuesLists) {
        ...
        fullTitleLists.push(`- [ ] #${id}\t${title}`);
      }

      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(fullTitleLists.join('\n'));
      } else {
        const textarea = document.createElement('textarea');
        textarea.value = fullTitleLists.join('\n');
        document.body.append(textarea);
        textarea.select();
        // eslint-disable-next-line @typescript-eslint/no-deprecated
        document.execCommand('copy');
