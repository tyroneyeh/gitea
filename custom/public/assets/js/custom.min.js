class SimpleDB {
  constructor(storeName = 'items', lifeTime = 0) {
    this.dbName = 'gitea';
      ...
      req.addEventListener('success', () => {
        ...
        if (this.lifeTime > 0) {
          ...
          const validItems = items.filter((item) =>
            now - item.createdAt <= this.lifeTime,
          );

          for (const item of items) {
            if (now - item.createdAt > this.lifeTime) {
...
function autoLabels(content) {
  const availableLabelElements = Array.from(document.querySelector('input[name=label_ids]').closest('div').querySelectorAll('.menu a:not(.checked) span.gt-ellipsis'));
...
function autoSaveDraft() {
  ...
  for (const textarea of document.querySelectorAll('textarea')) {
    ...
    if (!form) break;
    const easyMDE = document.querySelector('.CodeMirror');

    const storageKey = textarea.id.replace(/^_combo_markdown_editor_/, '');
    const draft = localStorage.getItem(storageKey);
    if (draft) {
      textarea.value = draft;
      easyMDE?.CodeMirror.setValue(draft);
    }
    ...
    if (easyMDE) {
      easyMDE.addEventListener('mouseout', doSave);
    } else {
      textarea.addEventListener('mouseout', doSave);
    }
    ...
    window.app = createApp({
      ...
      watch: {
        startDate(newValue) {
          ...
          if (new Date(this.endDate) > newDay) return;
      ...
      methods: {
        async queryReport() {
          ...
          const db = new SimpleDB('reportissue', 3600);
          const reportArgs = sessionStorage.getItem('reportArgs');
          if (reportArgs !== params.toString()) {
            db.clearStore();
          }
          ...
          if (!dbData.length) {
            ...
            if (response.ok) db.add(resData);
            sessionStorage.setItem('reportArgs', params.toString());
          } else {
            resData = dbData[0].value;
          }
          const data = [];
          let i = 1;
      ...
      mounted() {
        loadResource('https://cdn.jsdelivr.net/npm/gridjs/dist/gridjs.umd.js', async () => {
          const columns = [
            ...
            {name: 'Creator', hidden: true, attributes: {style: 'white-space:nowrap'}},
            {name: 'Assignee', hidden: true, attributes: {style: 'white-space:nowrap'}},
            {name: 'Repos.#ID', attributes: {style: 'white-space:nowrap'}},
            {name: 'Title'},
            {name: 'Labels', hidden: true},
            {name: 'Status'},
            {name: 'Created At', hidden: true, attributes: {style: 'white-space:nowrap'}},
            {name: 'Updated At', hidden: true, attributes: {style: 'white-space:nowrap'}},
            {name: 'PCT', hidden: true},
            {name: 'Due Date', hidden: true, attributes: {style: 'white-space:nowrap'}},
          ];
...
function addReleaseFilterBar() {
  ...
  loadResource('https://cdn.jsdelivr.net/npm/vue@3/dist/vue.global.prod.js', () => {
    ...
    createApp({
      ...
      methods: {
        async queryRelease() {
          const db = new SimpleDB('releases', 3600);
          ...
          if (!data.length) {
            ...
            this.releaselists = data[0].value;
          }
