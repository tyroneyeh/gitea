...
function addViewEventButton() {
  ...
  viewEventButton.className = 'attached header ui tw-mt-4 tw-ml-4';
  viewEventButton.textContent = `View logs (${events.length})`;
  viewEventButton.addEventListener('click', () => {
    for (const el of events) el.classList.toggle('tw-hidden');
    const action = viewEventButton.textContent.includes('View') ? 'Hide' : 'View';
    viewEventButton.textContent = `${action} logs (${events.length})`;
  });
...
function AddReverseButton() {
  ...
  reverseButton.className = 'attached header ui tw-mt-4 tw-ml-4';
