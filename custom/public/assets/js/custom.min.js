...
function hiToAssignee(userNames) {
  ...
  for (const availableUserElement of availableUserElements) {
    ...
    const id = availableUserElement.dataset.value;
    if (titleInputElement) {
      if (availableUserElement.classList.contains('checked') !== isChecked) {
        window.assigneeElement = assigneeElement;
        availableUserElement.click();
      }
    } else if (availableUserElement.classList.contains('checked') !== isChecked && !window.assigneeIds.includes(id)) {
      window.assigneeIds.push(id);
      post(assigneeElement.dataset.updateUrl, {action, id});
    }
  }
}
  ...
  for (const [label, pattern] of Object.entries(_autoLabels)) {
    if (pattern.test(content)) {
      ...
      if (contentLabelElement) {
        window.contentLabelElement = contentLabelElement;
        const action = 'attach';
        const id = contentLabelElement.dataset.value;
        await post(labelElement.dataset.updateUrl, {action, id});
        break;
      }
    }
  }
    ...
    const doSave = () => {
      ...
      if (content.length) {
        ...
        if (firstLine.startsWith('Hi @') && !firstLine.endsWith(',')) {
          ...
          if (window.assigneeElement) {
            window.assigneeElement.querySelector('.dropdown').click();
            window.assigneeElement = null;
          }
        }

        autoLabels(content);
        if (window.contentLabelElement) {
          contentLabelElement.click();
          window.contentLabelElement = null;
        }
      } else {
