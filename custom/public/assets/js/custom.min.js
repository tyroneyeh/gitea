...
function hiToAssignee(usernames, target) {
  ...
  for (const item of availableUsers) {
    ...
    if (usernames.includes(username) !== item.classList.contains('checked')) {
      item.click();
      found = true;
    }
  }
    ...
    const doSave = () => {
      ...
      if (content.length) {
        ...
        if (firstLine.startsWith('Hi @') && !firstLine.endsWith(',')) {
          hiToAssignee(firstLine.slice(4).split(' ').map((u) => u.trim().replace(/^@/, '')), target);
        }
...
function waitForTextareaToBeReady() {
  ...
  const intervalId = setInterval(() => {
    ...
    if (markdownEditor) {
      ...
      autoSaveDraft(markdownEditor); // Hi assignee, auto-labels
      eyesHandler();
