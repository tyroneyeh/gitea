...
async function autoLabels(content) {
  ...
  if (titleInputElement) {
    ...
    Array.from(document.querySelector('input[name=milestone_id]').closest('div').querySelectorAll('.menu a.item')).find((v) => v.textContent.trim() === new Date().getFullYear().toString())?.click();
  }
}
...
function relationPRtoIssuesTitle() {
  ...
  const fetchIssueDetails = async (mergeReposName, mergeIssueID) => {
    ...
    for (const labelId of issueData.labels.map((v) => v.id)) {
      labelElement.querySelector(`a[data-value="${labelId}"]`).click();
    }
  };
